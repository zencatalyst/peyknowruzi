// Linux version (Modified version for Windows available at https://github.com/Kasra-coder/magical-creations)
...
inline auto getCoords(  )
{
	...
	for ( unsigned int i = 0; i < numOfSymbols; ++i )
	{
		...
		if ( ch == '/' )
		{
			...
			coordVec[0] = --coordVec[0];
			coordVec[2] = --coordVec[2];
			coordVec[1] = ++coordVec[1];
			coordVec[3] = ++coordVec[3];
			uniquePtrs.up2ForwardSlash->setVectorOfCoords( coordVec );
		}
		else if ( ch == '\\' )
		{
