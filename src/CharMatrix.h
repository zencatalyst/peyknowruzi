...
public:
	...
	void setCharacterMatrix( const std::array<std::uint32_t, cartesian_components_count>& coordsOfChar ) const;
	...
	static auto createCharMatrix( const std::uint32_t Y_AxisLen = default_y_axis_len,
								  const std::uint32_t X_AxisLen = default_x_axis_len,
								  const char fillCharacter = default_fill_character );
